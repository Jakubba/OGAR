...
import { Search, Settings, Bell, User, LogOut, ChevronDown, Moon, Sun, Menu } from 'lucide-vue-next'
    ...
    <main class="flex-1 min-w-0 overflow-y-auto relative z-10 flex flex-col">
      ...
      >
        <button
          ...
          class="p-2.5 bg-white dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-500 dark:text-slate-400 hover:text-orange-600 dark:hover:text-orange-400 hover:border-orange-500/30 transition-all cursor-pointer shrink-0 lg:hidden"
          title="Otwórz menu"
        >
          <Menu class="w-4 h-4" />
        </button>
        ...
        <div class="flex items-center gap-1.5 sm:gap-3 shrink-0 ml-auto">
          ...
          <div ref="userMenuRef" class="relative flex items-center gap-2 sm:pl-2 sm:border-l border-slate-200 dark:border-slate-800">
            ...
            <div
              v-if="showUserMenu"
              class="absolute top-full right-0 mt-2 w-56 max-w-[calc(100vw-1.5rem)] bg-white dark:bg-[#0b1220] border border-slate-200 dark:border-slate-800 rounded-xl shadow-2xl shadow-black/10 dark:shadow-black/40 py-1.5 z-50"
            >
              <button
                @click="showUserMenu = false; goToSettings()"
                class="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
              >
                <Settings class="w-3.5 h-3.5" /> Ustawienia profilu
              </button>
              <button
                @click="showUserMenu = false; themeStore.requestToggleTheme()"
                class="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-xs font-semibold text-slate-600 dark:text-slate-300 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-slate-100 dark:hover:bg-slate-800/60 transition-colors cursor-pointer"
              >
                <Moon v-if="themeStore.mode === 'dark'" class="w-3.5 h-3.5" />
              ...
              <button
                ...
                class="w-full flex items-center gap-2.5 px-3.5 py-2.5 text-xs font-semibold text-rose-600 dark:text-rose-400 hover:bg-rose-500/10 transition-colors cursor-pointer"
              >
                <LogOut class="w-3.5 h-3.5" /> Wyloguj się
              </button>
            </div>
          </div>
        </div>
